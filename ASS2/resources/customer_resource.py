...
class CustomerRegister(Resource):
    ...
    parser.add_argument(
        "firstname", type=str, required=True, help="field cannot be lest blank"
    )
    parser.add_argument(
        "lastname", type=str, required=True, help="field cannot be lest blank"
    )

    parser.add_argument(
        "username", type=str, required=True, help="field cannot be lest blank"
    )
    parser.add_argument(
        "password", type=str, required=True, help="field cannot be lest blank"
    )
    ...
    def post(self):
        ...
        if user:
            return {
                "mesaage": f"A user with given username already exists having user id {user.id}"
            }, 400
